...
int EventLoop::CreateReadEvent(int fd, Handler readHandler) {
    if (!this->inited) {
        return SERVER_ERR;
    }
    if (fd > this->maxSize) {
    ...
    return this->fileEvents[fd].CreateReadEvent(readHandler);
}
int EventLoop::CreateWriteEvent(int fd, Handler writeHandler) {
    if (!this->inited) {
        return SERVER_ERR;
    }
    if (fd > this->maxSize) {
    ...
    return this->fileEvents[fd].CreateWriteEvent(writeHandler);
}
